...
**FR-1.1** The system must provide a user registration screen.

**FR-1.2** The system must provide a user login screen.

## FR-2 Repository Management

**FR-2.1** The system must display a list of all repositories owned by the authenticated user.

**FR-2.2** The system must allow the user to create a new repository.

## FR-3 Deck Display

**FR-3.1** The system must display the contents of the selected deck branch.

**FR-3.2** The system must allow the user to select the grouping mode used to display cards.

**FR-3.3** The system must support the following grouping modes:
...
**FR-3.4** The system must allow the user to select the ordering of cards within each group.

**FR-3.5** The system must support ordering by:

- card name,
- mana value,
- price in USD.

**FR-3.6** The system must allow the user to display cards in either text mode or image mode.

## FR-4 Deck Analysis

**FR-4.1** The system must display the deck's mana curve.

**FR-4.2** The system must display deck card counts.

**FR-4.3** The system must display a pie chart showing mana production and mana consumption by color.

**FR-4.4** The system must display the estimated total deck price in USD.

**FR-4.5** The system must compare deck card counts against the requirements of the selected
deck [format](#mtg-deck-format).

## FR-5 Card Tagging

**FR-5.1** The system must allow the user to assign any number of custom tags to each card within a repository.

**FR-5.2** The system must allow cards to be grouped by user-defined tags.

## FR-6 Card Search

**FR-6.1** The system must allow users to search for cards using the [Scryfall query syntax](#scryfall-query).

**FR-6.2** The system must retrieve search results using
the [Scryfall API](https://scryfall.com/docs/api/cards/search).

**FR-6.3** The system must allow cards from the search results to be added to the selected deck branch.

### Search Query Defaults

**FR-6.4** The system must allow the user to define a default query suffix appended automatically to
every [search query](#scryfall-query) within the repository.

## FR-7 Card Filtering

**FR-7.1** The system must allow the displayed deck contents to be filtered using
the [Scryfall query syntax](#scryfall-query).

**FR-7.2** The system must perform card filtering via [scryfall query syntax](#scryfall-query) locally without querying
the Scryfall API.

## FR-8 Branch Management

**FR-8.1** The system must allow the user to create, rename, delete, and switch between branches.

## FR-9 Branch Comparison

**FR-9.1** The system must allow the user to compare any two branches of a repository.

**FR-9.2** The system must allow the user to display either only differing cards or the complete contents of both
branches side by side.

**FR-9.3** The comparison view must support the same grouping modes as the standard deck view.

**FR-9.4** The system must designate one branch as the edited branch (the other as the comparison branch); editing card
quantities in the comparison view must modify only the edited branch.

## FR-10 Branch History

**FR-10.1** The system must let the user display the edit history of the selected branch.

**FR-10.2** The system must allow any historical revision to be compared with the current branch state
through [branch comparison](#fr-9-branch-comparison).

## FR-11 Deck Import

**FR-11.1** The system must import decks serialized in the MTGO format.

**FR-11.2** The system must import decks serialized in the MTGA format.

**FR-11.3** The system must import decks serialized using the Moxfield bulk edit format, including card tags.

## FR-12 Deck Export

**FR-12.1** The system must export decks in the following formats and must copy exported deck lists to the system clipboard:
...
The system provides standard usability features but does not need to include accessibility support for users with severe
...
Public-facing backend API documentation does not need to be provided.
